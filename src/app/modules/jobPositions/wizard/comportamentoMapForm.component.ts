...
export class ComportamentoMapFormComponent implements AfterViewInit {
    ...
    submit() {
        ...
        if (phase === 4) {
            classifications = parent.comportamentoAreaIdealClassification;
        } else if (phase === 5) {
            classifications = parent.comportamentoAreasPrincipaisClassifications;
        } else if (phase === 6) {
            classifications = parent.comportamentoAreasAlternativasClassifications;
        }
    ...
    validate() {
        let valid = false;
        this.parent.object.jobAreas.filter(a => this.parent.comportamentoAreaIdealClassification.indexOf(a.jobAreaClassification) > -1).map(a => valid = true);
        if (!valid) {
            this.parent.alert.buildAlert(0, 'Área ideal do comportamento precisa ser escolhida', 5);
            return false;
        }
        return true;
    }
