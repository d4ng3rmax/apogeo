...
@Component({
    selector: 'abordagemPrincipalForm',
    templateUrl: './abordagemPrincipalForm.component.html',
    styleUrls: ['../jobPosition.component.scss', '../../../app.component.scss'],
    encapsulation: ViewEncapsulation.None,
    providers: [JobPositionService, DepartmentService]
})
export class AbordagemPrincipalFormComponent implements AfterViewInit {
    ...
    async ngAfterViewInit() {
        if (this.parent.abordagens === undefined) {
            this.areas = await this.service.getAreas();
            this.parent.abordagens = this.areas;
            this.loadAbordagemSelections();

        } else {
            this.areas = this.parent.abordagens;
        }
    ...
    triggerAreaSelected() {
        this.principal = this.areas.filter(a => a.phase == 4)[0];
        this.principalOutros = this.areas.filter(a => a.phase == 5);
        this.alternativosOutros = this.areas.filter(a => a.phase == 6);
    }

    validate() {
        var phase = this.parent.phase;
        if (phase == 4 && this.areas.filter(a => a.phase == 4).length != 1) {
            this.parent.alert.buildAlert(0, 'Uma abordagem principal precisa ser selecionada', 4);
            return false;

        } else if ((phase == 5 || phase == 6) && this.areas.filter(a => a.phase == phase).length > 7) {
            this.parent.alert.buildAlert(0, 'Apenas 7 abordagens podem ser selecionadas', 4);
            return false;
        }
        return true;
    }
    ...
    loadAbordagemSelections() {
        if(this.parent.object.jobAreas === undefined) this.parent.object.jobAreas = [];
        for (let a of this.parent.object.jobAreas) {
            if (a.jobAreaClassification == "ABORDAGEM_PRINCIPAL") this.setSelectedAbordagem(a, 4);
            if (a.jobAreaClassification == "OUTRA_ABORDAGEM_PRINCIPAL_1") this.setSelectedAbordagem(a, 5);
            if (a.jobAreaClassification == "OUTRA_ABORDAGEM_PRINCIPAL_2") this.setSelectedAbordagem(a, 5);
            if (a.jobAreaClassification == "OUTRA_ABORDAGEM_PRINCIPAL_3") this.setSelectedAbordagem(a, 5);
            if (a.jobAreaClassification == "OUTRA_ABORDAGEM_PRINCIPAL_4") this.setSelectedAbordagem(a, 5);
            if (a.jobAreaClassification == "OUTRA_ABORDAGEM_PRINCIPAL_5") this.setSelectedAbordagem(a, 5);
            if (a.jobAreaClassification == "OUTRA_ABORDAGEM_PRINCIPAL_6") this.setSelectedAbordagem(a, 5);
            if (a.jobAreaClassification == "OUTRA_ABORDAGEM_PRINCIPAL_7") this.setSelectedAbordagem(a, 5);
            if (a.jobAreaClassification == "OUTRA_ABORDAGEM_ALTERNATIVA_1") this.setSelectedAbordagem(a, 6);
            if (a.jobAreaClassification == "OUTRA_ABORDAGEM_ALTERNATIVA_2") this.setSelectedAbordagem(a, 6);
            if (a.jobAreaClassification == "OUTRA_ABORDAGEM_ALTERNATIVA_3") this.setSelectedAbordagem(a, 6);
            if (a.jobAreaClassification == "OUTRA_ABORDAGEM_ALTERNATIVA_4") this.setSelectedAbordagem(a, 6);
            if (a.jobAreaClassification == "OUTRA_ABORDAGEM_ALTERNATIVA_5") this.setSelectedAbordagem(a, 6);
            if (a.jobAreaClassification == "OUTRA_ABORDAGEM_ALTERNATIVA_6") this.setSelectedAbordagem(a, 6);
            if (a.jobAreaClassification == "OUTRA_ABORDAGEM_ALTERNATIVA_7") this.setSelectedAbordagem(a, 6);
        }
        this.triggerAreaSelected();
    }

    setSelectedAbordagem(serverArea: any, phase: number) {
        var abordagem = this.parent.abordagens.filter(a => a.id == serverArea.areaProfissional.id)[0];
        abordagem.phase = phase;
        abordagem.selected = true;
    }
